...
class _netlocalD(nn.Module):
    def __init__(self, opt):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(opt.outch, opt.ndf, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
...
class _netlocalD16(nn.Module):
    def __init__(self, opt):
        super(_netlocalD16, self).__init__()
        self.ngpu = opt.ngpu
        self.main = nn.Sequential(
            ...
            nn.Conv2d(opt.ndf * 2, 1, 4, 1, 0, bias=False),
...
#            nn.Sigmoid()
    ...
    def forward(self, input):
        ...
        return output.view(-1, 1)    
class _netlocalD128(nn.Module):
    def __init__(self, opt):
        super(_netlocalD128, self).__init__()
        self.ngpu = opt.ngpu
        self.main = nn.Sequential(

            nn.Conv2d(opt.outch, opt.ndf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*2) x 64 x 64
            nn.Conv2d(opt.ndf * 2, opt.ndf * 4, 4, 2, 1, bias=False),
...
class _netlocalD256(nn.Module):
    def __init__(self, opt):
        super(_netlocalD256, self).__init__()
        self.ngpu = opt.ngpu
        self.main = nn.Sequential(
            ...
            nn.Conv2d(opt.ndf * 4, opt.ndf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(opt.ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*4) x 32 x 32
            nn.Conv2d(opt.ndf * 4, opt.ndf * 8, 4, 2, 1, bias=False),
...
#class _netlocalD64(nn.Module):
#    def __init__(self, opt):
#        super(_netlocalD64, self).__init__()
#        self.ngpu = opt.ngpu
#        self.main = nn.Sequential(
#            # input is (nc) x 256 x 256
##            nn.Conv2d(opt.outch, opt.ndf, 4, 2, 1, bias=False),
