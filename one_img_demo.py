...
parser.add_argument('--img_dir',default='none')  # please put and the name the SIR2 dataset here
...
def img_name(folder, pattern='/*'):
    filenames = sorted(glob.glob(folder + pattern))
    return filenames


RB_img_name = img_name(opt.img_dir)
...
imgs_data=cd.CustomDataset(RB_img_name)
...
imsave('./results/result_'+str(i)+'.png', fake_bg_cpu/255.)
